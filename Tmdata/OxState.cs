...
namespace FTI.Trialmax.Database
{
	...
	public class COxState : FTI.Trialmax.Database.CBaseRecord, ITmaxBaseObjectionRecord
	{
	...
	}// public class COxState : FTI.Trialmax.Database.CBaseRecord

	/// <summary>This class is used to manage a ArrayList of COxState objects</summary>
	public class COxStates : CBaseRecords
	{
		public enum eFields
		{
			UniqueId = 0,
		...
		public COxStates(CObjectionsDatabase tmaxDatabase) : base(tmaxDatabase)
		{
			m_strKeyFieldName = eFields.UniqueId.ToString();
		}

		/// <summary>This method allows the caller to add an object to the list</summary>
		/// <param name="oxState">Object to be added to the list</param>
		/// <returns>The object just added if successful, null otherwise</returns>
		public COxState Add(COxState oxState)
		{
			COxState oxAdded = null;

			if((oxAdded = (COxState)base.Add(oxState)) != null)
			{
		...
		public new COxStates Dispose()
		{
			return (COxStates)base.Dispose();
		...
		public new COxState this[int iIndex]
		{
			get { return (COxState)base[iIndex]; }

		}// public new COxState this[int iIndex]

		/// <summary>Gets the object located at the specified index</summary>
		/// <returns>Object at the specified index</returns>
		public new COxState GetAt(int iIndex)
		{
			return (COxState)base.GetAt(iIndex);
		}
		...
		{
			COxState oxState = (COxState)dxRecord;
			...
			strSQL += " VALUES(";
			strSQL += ("'" + SQLEncode(oxState.Label) + "',");
			strSQL += ("'" + oxState.CreatedOn.ToString() + "')");
		...
		public override string GetSQLUpdate(CBaseRecord dxRecord)
		{
			...
			strSQL += (eFields.Label.ToString() + " = '" + SQLEncode(oxState.Label) + "',");
			strSQL += (eFields.CreatedOn.ToString() + " = '" + oxState.CreatedOn.ToString() + "'");

			strSQL += " WHERE UniqueId = '{";
			strSQL += oxState.UniqueId.ToString();
			strSQL += "}';";
		...
		public COxState Find(System.Guid uniqueId)
		{
		...
		public COxState Find(string strValue , bool bLabel)
		{
			// Search for the requested object
			if(m_aList != null)
			{
				foreach(COxState O in m_aList)
				{
					if(bLabel == true)
					{
		...
		{
			return ((CBaseRecord)(new COxState()));
		}
		...
		protected override bool Exchange(CBaseRecord dxRecord,bool bSetFields)
		{
			...
			try
			{
				...
				if(bSetFields)
				{
					m_dxFields[(int)eFields.UniqueId].Value = oxState.UniqueId;
					m_dxFields[(int)eFields.Label].Value = oxState.Label;
					m_dxFields[(int)eFields.CreatedOn].Value = oxState.CreatedOn;
				}
				else
				{
					oxState.UniqueId = (System.Guid)(m_dxFields[(int)eFields.UniqueId].Value);
					oxState.Label = (string)(m_dxFields[(int)eFields.Label].Value);
					oxState.CreatedOn = (System.DateTime)(m_dxFields[(int)eFields.CreatedOn].Value);
				}
