...
{
	/// <summary>This class is used to manage an array of link records</summary>
	public class CDxLinks : CDxQuaternaries
	{
		...
		private CDxQuaternaries m_dxConnected = null;
		...
		public CDxLinks() : base()
		{
		}

		/// <summary>Overloaded constructor</summary>
		/// <param name="tmaxDatabase">Database connection that owns this collection</param>
		public CDxLinks(CTmaxCaseDatabase tmaxDatabase) : base(tmaxDatabase)
		{
		}
		...
		{
			CDxQuaternary dxConnected = null;
            ...
            m_dxConnected = new CDxQuaternaries(this.Database);
			
			//	Walk the parent chain of each record get to the actual application records
			foreach(CDxQuaternary O in this)
			{
			...
			foreach(CDxQuaternary O in m_dxConnected)
				this.AddList(O);
		...
		public override string GetSQLSelect()
		{
			...
			strSQL += " WHERE " + eFields.MediaType.ToString() + " = " + ((int)TmaxMediaTypes.Link).ToString();
			strSQL += " AND (";
			strSQL += (eFields.SourceId.ToString() + " = '" + strId + "'");
			strSQL += " OR ";
			strSQL += (eFields.SourceId.ToString() + " LIKE '" + strId + ".%')");
			strSQL += " ORDER BY " + eFields.TertiaryMediaId.ToString() + "," + eFields.DisplayOrder.ToString();
			strSQL += ";";
		...
		protected CDxQuaternary Connect(CDxQuaternary dxRecord, bool bSilent)
		{
			string			strId = "";
			long			lPrimaryId = 0;
			long			lSecondaryId = 0;
			bool			bSuccessful = false;
            CDxTertiaries   dxTertiaries = new CDxTertiaries(this.Database);
            CDxSecondaries  dxSecondaries = new CDxSecondaries(this.Database);
			
			while(bSuccessful == false)
			{
				//	Get the secondary id
				dxTertiaries.Fill(dxRecord.TertiaryMediaId);
				if(dxTertiaries.Count > 0)
				{
					Debug.Assert(dxTertiaries.Count == 1);
					lSecondaryId = dxTertiaries[0].SecondaryMediaId;
					dxTertiaries.Clear();
				}
				...
				//	Get the primary id
				dxSecondaries.Fill(lSecondaryId);
				if(dxSecondaries.Count > 0)
				{
					Debug.Assert(dxSecondaries.Count == 1);
					lPrimaryId = dxSecondaries[0].PrimaryMediaId;
					dxSecondaries.Clear();
				}
				...
				//	Format the unique record identifier
				strId =  (lPrimaryId.ToString() + ".");
				strId += (lSecondaryId.ToString() + ".");
				strId += (dxRecord.TertiaryMediaId.ToString() + ".");
			...
			if(bSuccessful == true)
                return (CDxQuaternary)(this.Database.GetRecordFromId(strId,bSilent));
			else
				return null;
		}
