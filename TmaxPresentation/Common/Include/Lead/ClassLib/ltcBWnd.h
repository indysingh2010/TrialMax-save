...
class LWRP_EXPORT LBitmapWindow :public LBitmap
{
   ...
   private :
      static   L_INT  EXT_CALLBACK MagGlassCS (HWND hWnd, L_INT nMaskPlaneStart, L_INT nMaskPlaneEnd, L_UCHAR *pMaskPlane, L_SIZE_T nMaskPlaneSize, L_VOID *pUserData);

   protected:
      virtual  L_INT    MagGlassCallBack(HWND hWnd, L_INT nMaskPlaneStart, L_INT nMaskPlaneEnd, L_UCHAR *pMaskPlane, L_SIZE_T nMaskPlaneSize);

   private :
      static  L_BOOL    m_bBitmapWindowRegisterd;
      static  L_INT     m_nBitmapWindowInstanceCount;
      static  HCURSOR   m_hHandCursor;
      static  HCURSOR   m_hPanCursor;
      static  HCURSOR   m_hZoomToRectCursor;
      static  HCURSOR   m_hZoomInMouseCursor;
      static  HCURSOR   m_hRegionCursor;
      static  HCURSOR   m_hZoomViewCursor;

      LDoubleBuffer     m_DoubleBuffer;
      LShape*           m_pShape;
      L_INT             m_nHScrollPos;
      L_INT             m_nHorzMaxPos;
      L_INT             m_nVScrollPos;
      ...
      LBuffer           m_LBuffer;
      LDoubleBuffer     m_LDoubleBuffer;
      L_INT             m_nNotUsed5;
      L_INT             m_nStopPaint;
      L_BOOL            m_bOperationCanceld;

      //MagGlass
      MAGGLASSOPTIONS   m_MagGlassOptions;
      L_BOOL            m_bMagGlass;
   ...
   private :
      ...
      static L_VOID    EXT_CALLBACK PanWindowCS(HWND hPanWnd,HWND hParentWnd,L_UINT uFlag, 
                                    LPRECT lprcPan, L_VOID * pUserData);
      ...
      static L_INT      EXT_CALLBACK PaintWhileLoadCallBack(pFILEINFO pFileInfo,pBITMAPHANDLE pBitmap,
                                               ...
                                               L_INT nRow,L_INT nLines,L_VOID * pData);
      ...
      L_INT             UpdateMagGlassRect(RECT * prcDst);
   ...
   public :
      ...
      virtual L_INT        GetMagGlassOptions(MAGGLASSOPTIONS *pOptions, L_UINT uStructSize);
      virtual L_INT        SetMagGlassOptions(MAGGLASSOPTIONS *pOptions=NULL);
      ...
      virtual HCURSOR      SetCursor(HCURSOR hCursor, L_UINT uToolType = TOOL_USERMODE);
      virtual LBitmap   *  SetFloater (LBitmap * pFloater);
private:
       L_INT        XClientToBitmap(L_INT x);
       L_INT        YClientToBitmap(L_INT y);
       L_INT        XClientToBitmapInverse(L_INT x);
       L_INT        YClientToBitmapInverse(L_INT y);
       L_VOID       PixelizePoint(LPPOINTS pPoint);

public:
       L_BOOL       HasZoomView();
       L_INT        AddZoomView(pZOOMVIEWPROPS pZoomViewProps);
       L_INT        GetZoomViewProps(pZOOMVIEWPROPS pZoomViewProps, L_UINT32 uStructSize);
       L_INT        UpdateZoomView(pZOOMVIEWPROPS pZoomViewProps);
       L_INT        DestroyZoomView(L_UINT uIndex);
       L_UINT       GetZoomViewsCount();
       L_BOOL       EnableHiliteZoomView(L_BOOL bEnable);
       L_BOOL       IsHiliteZoomViewEnabled();

private:
      POINT			ChangeToZoomPoint(POINT *pt);

public :
   L_INT UpdateMagGlassShape(L_UINT uMagGlassShape, L_HRGN hMagGlassRgn);
};
