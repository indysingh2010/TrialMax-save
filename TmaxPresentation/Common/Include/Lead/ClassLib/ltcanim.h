...
{  
   ...
   protected:
      L_INT   GetAt(L_UINT nIndex, LBitmapBase  * pBitmap, L_BOOL bReflectIndex = FALSE);
      L_INT   SetAt(L_UINT nIndex, LBitmapBase  * pBitmap, L_BOOL bReflectIndex = FALSE);
      L_INT   InsertFrame(LBitmapBase  * pLBitmap, L_UINT uIndex =-1 /*last item*/);
      L_INT   RemoveFrame(L_UINT uIndex, LBitmapBase  * pBitmap = NULL);
   ...
   public   :
      ...
      LAnimationWindow();
      LAnimationWindow(HWND hWndParent,LBitmapList  * pBitmapList=NULL,
      ...
      L_INT    GetPosition(L_INT  * pLeft, L_INT  * pTop, L_UINT nIndex) const;
      
      L_INT    EnableTransparency(L_BOOL bEnable = TRUE, COLORREF clrTransparent = RGB(0,0,0), L_UINT nIndex  = ANIM_ALL_ITEMS);
      L_INT    IsTransparencyEnabled(L_BOOL  * pEnabled, COLORREF  * pTransparentColor,L_UINT nIndex) const;
...
public : 
   L_VOID  SetBitmapList(LBitmapList * pBitmapList,LBitmapList * pBitmapListPrev=NULL, L_INT nBitsPerPixel=0, L_INT nColorOrder=ORDER_BGRORGRAY);
   ...
   L_INT   Load(  L_INT nBitsPerPixel=0, 
                  ...
                  pFILEINFO pFileInfo = NULL);
   //3
   L_INT   Load ( L_TCHAR * pszFileName,
                  L_INT nBitsPerPixel, 
                  L_INT nColorOrder, 
                  L_INT nPage,
                  pFILEINFO pFileInfo = NULL);
   ...
   L_BOOL   DoEvents(L_BOOL * pbCanceled = NULL, L_UINT uVKey = VK_ESCAPE)  const;
