...
class LWRP_EXPORT LVectorWindow :public LVectorBase
{
   ...
   private :
      ...
      L_INT             SetEngine(L_UINT32 dwFlags);
  ...
  */
   ...
   public :
      ...
      virtual L_INT     CreateNew( L_UINT32 dwFlags = VECTOR_ENGINE_DOUBLEBUFFER);

      L_INT             SelectAllObjects( L_BOOL bSelect,RECT *pRect);

      L_BOOL            SetSelectedOnly( L_BOOL bSelectedOnly);
      L_BOOL            GetSelectedOnly();
      L_INT             SetPan(const POINT *pPan);

      L_BOOL            IsControl();

      L_INT             SetBackgroundColor(L_COLOR color=0);
      L_INT             GetBackgroundColor(L_COLOR *pColor);
